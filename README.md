...
⚠️ **주의**
- 하위 전략의 기간 파라미터(듀얼 모멘텀 46/94 등)는 탐색하지 않았다. **의도적이다** —
  기간까지 탐색 축을 넓혀(조합 9개 → 162개) 다시 돌렸더니 아웃샘플이
  +76.4% → +56.7%, 비용 2배에서 +45.1% → +9.0%로 **나빠졌다**.
  탐색 공간을 넓히면 과거 잡음에 맞는 조합을 찾아낼 뿐이다. (`docs/REWORK_PLAN.md` A.7)
